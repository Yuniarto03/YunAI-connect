...
import {
  ...
  SavedPivotSummary,
  ActivePivotView
  // MaximizedPivotViewModalProps removed from types import
} from '../types';
...
import { RAW_COLOR_VALUES, CHART_COLOR_PALETTE, GEMINI_TEXT_MODEL } from '../constants';
...
import { interpretUiCommand } from '../services/geminiService'; // For AI UI command interpretation
...
// Simplified: Only PivotValueFieldConfig as CalculatedMeasures are removed.
...
// Define MaximizedPivotViewModalProps locally
interface MaximizedPivotViewModalProps {
  ...
  renderActualPivotTable: Function; 
  getRenderableRowNodes: Function;
  getLeafColNodesForData: Function;
  toggleExpand: Function;
  handleCollapseAll: Function;
  handleExpandAll: Function;
  exportPivotToExcel: Function; 
  setChartModalViewId: Function; 
  setShowDisplayOptionsModalForViewId: Function; 
...
export const PivotTable: React.FC = () => { 
  ...
  const {
    ...
    reduceMotion,
    // setIsSidebarOpen, // Removed: No longer in AppContextType
    pivotDataIdentifier,
  ...
  const hasInitializedForData = useRef<string | null>(null); 
  const lastCalculatedViewRef = useRef<{id: string | null, config: PivotConfig | undefined, options: PivotOptions | undefined}>({id: null, config: undefined, options: undefined});
  const lastSuccessfulCalculationRef = useRef<{dataIdentifier: string | null, config: string, options: string} | null>(null);
  const [isConfigPanelMinimized, setIsConfigPanelMinimized] = useState(false);
  ...
  useEffect(() => {
    if (processedData) {
      ...
      if (hasInitializedForData.current !== pivotDataIdentifier) {
        ...
        setIsConfigPanelMinimized(false); // Ensure config panel is expanded for new data
        ...
        hasInitializedForData.current = pivotDataIdentifier; 
      }
    } else {
      setAllAvailableFields([]);
    }
    if (!processedData) hasInitializedForData.current = null;
  ...
  const calculatePivotForView = useCallback(async (viewToCalculate: ActivePivotView | undefined) => {
    ...
    const currentConfigString = JSON.stringify(viewToCalculate.pivotConfig);
    const currentOptionsString = JSON.stringify(viewToCalculate.pivotOptions);
    
    if (lastSuccessfulCalculationRef.current?.dataIdentifier === pivotDataIdentifier &&
        lastSuccessfulCalculationRef.current?.config === currentConfigString &&
        lastSuccessfulCalculationRef.current?.options === currentOptionsString) {
      // Already calculated this exact state for this data, skip.
      return; 
    }
    ...
    try {
      ...
      lastSuccessfulCalculationRef.current = { dataIdentifier: pivotDataIdentifier, config: currentConfigString, options: currentOptionsString };
    } catch (error) { 
  ...
  }, [processedData, updateActivePivotViewResult, pivotDataIdentifier]);

  useEffect(() => {
    if (processedData) {
      setIsLoadingGlobal(true);
      const viewsToRecalculate = [...activePivotViews]; 
      Promise.all(viewsToRecalculate.map(view => calculatePivotForView(view)))
        .finally(() => setIsLoadingGlobal(false));
    }
  }, [processedData, calculatePivotForView]); 
  ...
  useEffect(() => {
    ...
    if (!viewToProcess) {
        lastCalculatedViewRef.current = { id: null, config: undefined, options: undefined };
        return;
    }

    const idChanged = viewToProcess.id !== lastCalculatedViewRef.current.id;
    
    // Use stringify for robust content comparison
    const stringifiedCurrentConfig = JSON.stringify(viewToProcess.pivotConfig);
    const stringifiedCurrentOptions = JSON.stringify(viewToProcess.pivotOptions);
    const stringifiedLastConfig = JSON.stringify(lastCalculatedViewRef.current.config);
    const stringifiedLastOptions = JSON.stringify(lastCalculatedViewRef.current.options);

    const configChanged = stringifiedCurrentConfig !== stringifiedLastConfig;
    const optionsChanged = stringifiedCurrentOptions !== stringifiedLastOptions;

    if (!loadingViewIds.has(viewToProcess.id) && (idChanged || configChanged || optionsChanged)) {
      lastCalculatedViewRef.current = {
          id: viewToProcess.id,
          config: JSON.parse(stringifiedCurrentConfig), // Store a deep copy of the config that led to this calculation
          options: JSON.parse(stringifiedCurrentOptions), // Store a deep copy of the options
      };
      calculatePivotForView(viewToProcess);
    }
  }, [currentEditingPivotViewId, activePivotViews, calculatePivotForView, loadingViewIds, processedData]);
  ...
  useEffect(() => {
    if (isPivotPresentationViewActive) {
        ...
        // setIsSidebarOpen(false); // Sidebar no longer exists
    ...
    const handleEscKey = (event: KeyboardEvent) => { if (event.key === 'Escape' && isPivotPresentationViewActive) { setIsPivotPresentationViewActive(false); /* setIsSidebarOpen(true); */ }}; // Sidebar no longer exists
    window.addEventListener('keydown', handleEscKey);
    return () => window.removeEventListener('keydown', handleEscKey);
  }, [isPivotPresentationViewActive, currentEditingPivotViewId, activePivotViews /* Removed setIsSidebarOpen */]);
    ...
    <div className={`p-4 md:p-6 h-full flex flex-col ${theme.textColor} futuristic-scrollbar overflow-y-auto`}>
      ...
      <div className={`${theme.cardBg} p-4 rounded-lg shadow-xl border ${theme.borderColor} mb-6 sticky top-[calc(var(--header-height,60px)_+_1rem)] z-50`}>
        <div className="flex justify-between items-center mb-2">
          <h3 className={`text-lg font-semibold text-${theme.accent4}`}>Configuring: <span className="italic">{currentEditingView?.name || "No Pivot Selected"}</span></h3>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setIsConfigPanelMinimized(!isConfigPanelMinimized)}
            title={isConfigPanelMinimized ? "Expand Configuration Panel" : "Minimize Configuration Panel"}
            className="!p-1"
            aria-label={isConfigPanelMinimized ? "Expand Configuration Panel" : "Minimize Configuration Panel"}
          >
            {isConfigPanelMinimized ? <ChevronDown size={20} /> : <ChevronUp size={20} />}
          </Button>
        </div>
         
        <div className={`${reduceMotion ? '' : 'transition-all duration-300 ease-in-out'} overflow-hidden ${isConfigPanelMinimized ? 'max-h-0 opacity-0' : 'max-h-[1000px] opacity-100'}`}>
          <div className="pt-2">
            { !currentEditingView && !isConfigPanelMinimized && <p className="text-xs text-orange-400 mb-2">Select a pivot view below or add a new one to start configuring.</p>}
...
const renderDropZone = (title: string, IconElement: React.ElementType | null, area: PivotArea, fields: PivotFieldConfig[] | PivotValueFieldConfig[], currentEditingView?: ActivePivotView, theme?:Theme, handleDragOverFn?: any, handleDropFn?: any, setIsAddCalculatedMeasureModalOpenFn?: Function, removeFieldFromDragAreaFn?:Function, handleValueAggregationChangeFn?:Function, handleDragStartFn?:Function) => {
    ...
    if (area === 'rows') {
      itemsToRender = (fields as PivotFieldConfig[]).map((item, idx) => 
        renderFieldItem(item.field, area, idx, false, currentEditingView, theme, handleDragStartFn, removeFieldFromDragAreaFn, handleValueAggregationChangeFn)
      );
      ...
      itemsToRender = (fields as PivotValueFieldConfig[]).map((item, idx) => 
        renderFieldItem(item.field, area, idx, false, currentEditingView, theme, handleDragStartFn, removeFieldFromDragAreaFn, handleValueAggregationChangeFn)
      );
      itemsToRender.push(...(configForArea.calculatedMeasures || []).map(cm => 
        renderFieldItem(cm.name, area, undefined, true, currentEditingView, theme, handleDragStartFn, removeFieldFromDragAreaFn, handleValueAggregationChangeFn)
      ));
    }

    return ( 
        <div 
            className={`p-2 border-2 border-dashed ${theme.borderColor} rounded-lg min-h-[60px] h-full flex flex-col bg-${theme.darkGray}/30 futuristic-scrollbar overflow-y-auto hover:border-${theme.accent1} transition-colors duration-150`} 
            onDragOver={handleDragOverFn} 
            onDrop={() => handleDropFn && handleDropFn(area)}
            aria-label={`${title} drop zone`}
        >
            {IconElement && (<div className="flex items-center mb-1.5"><IconElement size={14} className={`mr-1.5 text-${theme.accent2} flex-shrink-0`}/><h4 className={`text-xs font-semibold text-${theme.accent2}`}>{title}</h4></div>)}
            {!IconElement && title && <h4 className={`text-xs font-semibold text-${theme.accent2} mb-1.5`}>{title}</h4> }
            
            {itemsToRender.length > 0 ? itemsToRender : (
                <p className={`text-[11px] text-center ${theme.textColor} opacity-50 mt-1 flex-grow flex items-center justify-center`}>Drop fields here</p>
            )}
            
            {area === 'values' && setIsAddCalculatedMeasureModalOpenFn && (
                <Button variant="ghost" size="sm" onClick={() => setIsAddCalculatedMeasureModalOpenFn(true)} className={`w-full mt-1.5 !text-xs !py-1 text-${theme.accent3} border border-dashed border-${theme.accent3}/50 hover:bg-${theme.accent3}/10`} leftIcon={<Calculator size={12}/>}>
...
const InlineFilterValueSelector: React.FC<InlineFilterValueSelectorProps> = ({view, filterItem, filterIndex, handleFilterValueChange, currentEditingViewId}) => {
    ...
    const [searchTerm, setSearchTerm] = useState(''); const dropdownRef = useRef<HTMLDivElement>(null); // Removed useState for openFilterValueDropdown and searchTerm
